...
namespace HAL {
    ...
    uint8_t num_broken_pins = 0;
    /* In case a singlular pin is broken, we can gracefully handle it without too much issue */
    #ifdef ENCODER_A_PIN_BROKEN
        num_broken_pins += 1;
        uint8_t* revEncMap = revEncMap_A_Pin_Broken;
    #endif
    #ifdef ENCODER_B_PIN_BROKEN
        num_broken_pins += 1;
        uint8_t* revEncMap = revEncMap_B_Pin_Broken;
    #endif
    #ifdef ENCODER_C_PIN_BROKEN
        num_broken_pins += 1;
        uint8_t* revEncMap = revEncMap_C_Pin_Broken;
    #endif
    if num_broken_pins > 1 {
        "At most one encoder pin can be broken"
        exit(1);
    }
