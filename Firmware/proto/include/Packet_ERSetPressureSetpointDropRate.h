...
#define PACKET_ID_ERSetPressureSetpointDropRate 213

class PacketERSetPressureSetpointDropRate 
{
public:
    ...
    static PacketERSetPressureSetpointDropRate fromRawPacket(Comms::Packet *packet)
    {
        float v_m_Value = Comms::packetGetFloat(packet, 0);
        return PacketERSetPressureSetpointDropRate(v_m_Value);
    }
...
{    
    ...
    PacketERSetPressureSetpointDropRate build() const    
    {            
        static_assert( f0set, "All fields must be set before building."); // Added static assert    
        return PacketERSetPressureSetpointDropRate(this->m_Value);    
    }    
...
private:
    ...
    PacketERSetPressureSetpointDropRate(float ValueIn)
        : m_Value(ValueIn)
    {}
};
