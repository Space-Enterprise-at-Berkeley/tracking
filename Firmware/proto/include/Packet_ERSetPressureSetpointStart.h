...
#define PACKET_ID_ERSetPressureSetpointStart 212

class PacketERSetPressureSetpointStart 
{
public:
    ...
    static PacketERSetPressureSetpointStart fromRawPacket(Comms::Packet *packet)
    {
        float v_m_Value = Comms::packetGetFloat(packet, 0);
        return PacketERSetPressureSetpointStart(v_m_Value);
    }
...
{    
    ...
    PacketERSetPressureSetpointStart build() const    
    {            
        static_assert( f0set, "All fields must be set before building."); // Added static assert    
        return PacketERSetPressureSetpointStart(this->m_Value);    
    }    
...
private:
    ...
    PacketERSetPressureSetpointStart(float ValueIn)
        : m_Value(ValueIn)
    {}
};
