...
#define PACKET_ID_SetCalibrationMultiplier 105

class PacketSetCalibrationMultiplier 
{
public:
    ...
    static PacketSetCalibrationMultiplier fromRawPacket(Comms::Packet *packet)
    {
        uint8_t v_m_Channel = Comms::packetGetUint8(packet, 0);
        float v_m_Value = Comms::packetGetFloat(packet, 1);
        return PacketSetCalibrationMultiplier(v_m_Channel, v_m_Value);
    }
...
{    
    ...
    PacketSetCalibrationMultiplier build() const    
    {            
        static_assert( f0set &&  f1set, "All fields must be set before building."); // Added static assert    
        return PacketSetCalibrationMultiplier(this->m_Channel, this->m_Value);    
    }    
...
private:
    ...
    PacketSetCalibrationMultiplier(uint8_t ChannelIn, float ValueIn)
        : m_Channel(ChannelIn), m_Value(ValueIn)
    {}
};
