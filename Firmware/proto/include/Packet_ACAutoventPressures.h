...
#define PACKET_ID_ACAutoventPressures 5

class PacketACAutoventPressures 
{
public:
    ...
    static PacketACAutoventPressures fromRawPacket(Comms::Packet *packet)
    {
        ...
        return PacketACAutoventPressures(v_m_NosAutoventPressure, v_m_IpaAutoventPressure, v_m_NoCommsAbortEnabled);
    }
...
{    
    ...
    PacketACAutoventPressures build() const    
    {            
        static_assert( f0set &&  f1set &&  f2set, "All fields must be set before building."); // Added static assert    
        return PacketACAutoventPressures(this->m_NosAutoventPressure, this->m_IpaAutoventPressure, this->m_NoCommsAbortEnabled);    
    }    
...
private:
    ...
    PacketACAutoventPressures(float NosAutoventPressureIn, float IpaAutoventPressureIn, uint8_t NoCommsAbortEnabledIn)
        : m_NosAutoventPressure(NosAutoventPressureIn), m_IpaAutoventPressure(IpaAutoventPressureIn), m_NoCommsAbortEnabled(NoCommsAbortEnabledIn)
    {}
};
