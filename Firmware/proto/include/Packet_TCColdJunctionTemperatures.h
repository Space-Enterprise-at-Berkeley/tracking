...
#define PACKET_ID_TCColdJunctionTemperatures 3

class PacketTCColdJunctionTemperatures 
{
public:
    ...
    static PacketTCColdJunctionTemperatures fromRawPacket(Comms::Packet *packet)
    {
        ...
        return PacketTCColdJunctionTemperatures(v_m_Values);
    }
...
{    
    ...
    PacketTCColdJunctionTemperatures build() const    
    {            
        static_assert( f0set, "All fields must be set before building."); // Added static assert    
        return PacketTCColdJunctionTemperatures(this->m_Values);    
    }    
...
private:
    ...
    PacketTCColdJunctionTemperatures(std::array<float, 8> ValuesIn)
        : m_Values(ValuesIn)
    {}
};
